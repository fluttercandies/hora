...
/// Locale registry for dynamic locale loading (optional).
///
/// Most users should directly use locale instances:
/// ```dart
/// import 'package:hora/hora.dart';
///
/// // Direct usage - tree-shakable
/// final h = Hora.now(locale: const HoraLocaleZhCn());
/// ```
///
/// For dynamic locale switching by code, register locales first:
/// ```dart
/// import 'package:hora/hora.dart';
...
///
/// void main() {
///   // Register only the locales you need
///   HoraLocales.register(const HoraLocaleJa());
///   HoraLocales.register(const HoraLocaleKo());
///
///   // Then use by code
///   final locale = HoraLocales.get('ja');
/// }
/// ```
class HoraLocales {
  HoraLocales._();

  static final Map<String, HoraLocale> _locales = {};

  /// Registers a locale for dynamic lookup.
  static void register(HoraLocale locale) {
    _locales[locale.code.toLowerCase()] = locale;
  }

  /// Gets a locale by code. Returns null if not registered.
  static HoraLocale? get(String code) => _locales[code.toLowerCase()];

  /// Gets a locale by code, or returns fallback if not found.
  static HoraLocale getOrDefault(String code, HoraLocale fallback) =>
      _locales[code.toLowerCase()] ?? fallback;

  /// All registered locale codes.
  static Iterable<String> get codes => _locales.keys;

  /// Whether a locale is registered.
  static bool has(String code) => _locales.containsKey(code.toLowerCase());

  /// Total number of registered locales.
  static int get count => _locales.length;

  /// Clears all registered locales.
  static void clear() => _locales.clear();
}
